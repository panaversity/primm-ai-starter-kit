...
| 50 | 3 | if/elif/else branches, for loops with range()/enumerate(), while loops, break/continue/pass, .append()/.pop()/.sort()/.remove() on lists, nested loops, branch coverage testing |
| 51 | 3 | @dataclass decorator, class keyword, typed fields, default values, field(default_factory=list), frozen=True, nested dataclasses, object instantiation, attribute access (dot notation vs dict brackets) |
| 52 | 3 | Arrange-Act-Assert pattern, @pytest.fixture, conftest.py, @pytest.mark.parametrize, pytest.raises, test coverage with pytest --cov, test markers |
